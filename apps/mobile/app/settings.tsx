...
export default function Settings(){
  const router=useRouter(),[refreshing,setRefreshing]=useState(false),[updateStage,setUpdateStage]=useState<UpdateStage>('idle')
  const version=Constants.expoConfig?.version??'—'
  async function checkUpdate(){setRefreshing(true);try{await checkAndInstallUpdate(setUpdateStage)}finally{setRefreshing(false)}}
  return <SafeAreaView style={common.screen} edges={['top','left','right']}><ScrollView contentContainerStyle={common.content} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={()=>void checkUpdate()} tintColor={colors.blue} colors={[colors.blue]}/> }>
    ...
    <View style={common.card}><Text style={{color:colors.text,fontWeight:'900',fontSize:17}}>Atualização do aplicativo</Text><Text style={[common.muted,{marginTop:7,marginBottom:14}]}>Versão instalada: {version} • {updateStageLabel[updateStage]}</Text><Pressable style={[common.button,updateStage!=='idle'&&{opacity:.75}]} onPress={()=>void checkUpdate()} disabled={updateStage!=='idle'}><Ionicons name="cloud-download-outline" size={19} color="white"/><Text style={common.buttonText}>{updateStageLabel[updateStage]}</Text></Pressable></View>
